<template>
  <div>
    <b-modal :id="idModalPaymentControl" centered title="Mensaje de Pago" no-close-on-backdrop no-close-on-esc
      hide-header-close size="lg" hide-footer no-fade>
      <b-row>
        <b-col cols="3" class="text-center pt-3 spinner-container">
          <div v-if="!isChangeStatus" class="spinner"></div>
          <font-awesome-icon v-else :icon="['fas', 'exclamation-triangle']" size="4x" :style="{ color: 'orange' }" />
        </b-col>
        <!-- Monto -->
        ...
        <b-col colos="12" class="text-center font-italic">
          <h2 v-show="!isChangeStatus" class="text-primary py-5 pb-2" style="white-space: pre-line;">{{ msg }}</h2>
          <h2 v-show="isChangeStatus" class="text-danger py-1 pb-4 pt-4" style="white-space: pre-line;">{{ msgError }}
          </h2>
        </b-col>
