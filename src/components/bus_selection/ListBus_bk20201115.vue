<template>
  <div role="tablist">
    ...
    <!-- List bus when other -->
    <b-card v-else no-body class="mb-1" v-for="bus in schedules" :key="bus.idServicio">
      <b-card-header header-tag="header" class="p-1" role="tab">
        <b-button
          block href="#"
          v-b-toggle="['accordion', buttonType, bus.idServicio].join('-')"
          class="bg-light text-dark pt-3"
          @click="changeColorT([buttonType, bus.idServicio].join('-'))"
          :id="['list', buttonType, bus.idServicio].join('-')"
          :name="['list', buttonType, bus.idServicio].join('-')"
          :ref="['list', buttonType, bus.idServicio].join('-')"
          style="height: 130px"
        >
          <b-row class="font-weight-lighter textHeaders">
            <b-col cols="2" class="text-left">
              <b-img
                thumbnail
                :src="['data:image/jpeg;base64,',bus.logo].join('')"
            ...
            <b-col
              cols="2"
              :ref="['station',buttonType,bus.idServicio].join('-')"
            ...
            >
              <span
                ...
                :id="['price',buttonType,bus.idServicio].join('-')"
                :ref="['price',buttonType,bus.idServicio].join('-')"
                :data-price-1="bus.tarifaPrimerPiso"
                :data-price-2="(bus.tarifaSegundoPiso)?bus.tarifaSegundoPiso:''"
              >
                <h5>${{bus.tarifaPrimerPiso}}</h5>
                <h5>{{(bus.tarifaSegundoPiso) ? ['$',bus.tarifaSegundoPiso].join('') : ''}}</h5>
              </span>
            </b-col>
            ...
            <b-col cols="4" class="text-right">
              <h4>
                <b-badge
                  v-show="false"
                  variant="success"
                  :ref="['badge-selected',buttonType,bus.idServicio].join('-')"
                >
      ...
      <b-collapse
        :id="['accordion',buttonType,bus.idServicio].join('-')"
        :visible="false"
        accordion="my-accordion"
        role="tabpanel"
        :ref="['accordion',buttonType,bus.idServicio].join('-')"
      >
        <b-card-body>
          ...
          <b-row>
            <b-col cols="12">
              ...
              <!-- Bus Seat -->
              <bus-seat
                v-if="(activeButton === [buttonType,bus.idServicio].join('-'))"
                v-bind="{
                  type: buttonType,
                  name: [buttonType,bus.idServicio].join('-'),
                  active: activeButton,
